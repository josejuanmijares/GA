...
module myGA3
	...
	type population
		...
		function population(N::Int64, M::Int64, mypids = workers())
			...
			this.M = M
			this.NSamples = 0
			this.x = SharedArray(Float32,(N,M), init=S->S[localindexes(S)]=rand(length([localindexes(S);])), pids = mypids);
			this.scores = SharedArray(Float32, M, init= S2->S2[localindexes(S2)]=zeros(Float32,length([localindexes(S2);])), pids = mypids);
